...
exports.newPost = (req, res)=>{
    let {user_id, title, description, image1, image2, image3, image4, contact, email} = req.body
    ...
    const uploadSql = "INSERT INTO posts (user_id, title, description, image1, image2, image3, image4, contact, email) Values(?,?,?,?,?,?,?,?,?)"
    pool.getConnection((err, connnection)=>{
        if(err) return res.status(500).json({msg:'server error posting to classifieds'})
        connnection.query(uploadSql, [user_id, title, description, image1, image2, image3, image4, contact, email], (err)=>{
            connnection.release()
